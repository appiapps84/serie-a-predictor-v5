...
// =========================================================
    // 9. SALVA RISULTATI SU DATABASE
    // =========================================================

    if (storedMatches.length > 0) {
      try {
        await fetch(
          `${process.env.VERCEL_URL || 'http://localhost:3000'}/api/sync-result`,
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ storedMatches })
          }
        );
      } catch (error) {
        console.warn('sync-result call failed (non-blocking):', error);
      }
    }
