...
/* =========================================================
   UNDERSTAT - fetch DIRETTO (il corpo HTML serve intero)
========================================================= */
  ...
  function ensure(team) {
    ...
    if (!stats[key]) {
      stats[key] = {
        ...
        played: 0, xgFor: 0, xgAgainst: 0,
        scored: 0, conceded: 0,
        homePlayed: 0, homeXgFor: 0,
        awayPlayed: 0, awayXgFor: 0,
        matchesWithXg: 0
      };
    }
    return stats[key];
...
async function fetchUnderstatStats() {
  ...
  try {
    ...
    if (!html || html.length < 5000) {
      return { available: false, reason: "risposta troppo corta (possibile blocco)", year, stats: {} };
    }

    const datesData = parseUnderstatJsonVar(html, "datesData");

    if (!datesData) {
      return { available: false, reason: "datesData non trovato nel HTML", year, stats: {} };
    }
...
/* =========================================================
   SALVATAGGIO RISULTATI SU SUPABASE
========================================================= */
...
export default async function handler(req, res) {
  ...
  const storedUrl =
    ...
    `&status=finished&limit=${STORED_MATCH_LIMIT}&sort=desc`;
